...
func TestAPRSPacketFeed_TransmitPaths(t *testing.T) {
	...
	g.sendMessage(context.Background(), &aprsOutbound{msg: &transport.MeshMessage{
		From:        0x0a0b0c0d,
	...
	g.sendMessage(context.Background(), &aprsOutbound{msg: &transport.MeshMessage{DecodedText: "PING 1", Destination: "MSPRLX-10", RawText: true}})
	rec = c.wait(t)
	if rec.Text != "PING 1" {
	...
	g.sendMessage(context.Background(), &aprsOutbound{msg: &transport.MeshMessage{DecodedText: "AAAA", Encrypted: true}})
	rec = c.wait(t)
	if rec.Text != "" {
