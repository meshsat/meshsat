...
**Parallax's RTL-SDR is off the bus.** It failed re-enumeration at 12:21 and stayed dead through a host-controller reset and two ganged root VBUS cuts (5 s and 25 s, bridge stopped around them, every other device came back clean, the modem within a minute). Physical re-seat or replacement (MESHSAT-1001). Consequences already landed (f275ecc): the spectrum monitor attaches a dongle that appears after startup, the `rtl_sdr` health target always exists, and its hard rung asks for the hub-port cut before the sysfs reset. Two more StarTech hubs arrive 17 Sep (MESHSAT-1002) so RTL-SDR and GPS move off the Pi root ports, which the Pi 5 can only switch as one group.
