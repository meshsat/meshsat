...
| `docker-patches/rtl_tcp-exit-on-signal.patch` | Found by the first level-1 heal on tesseract after the deploy (12:33): `rtl_tcp` swallows a SIGTERM that lands while a client is connected (it ends the session, resets `do_exit` and listens again), so the stop fell through to SIGKILL and `rtlsdr_close` never ran. A real signal now ends the process. |
